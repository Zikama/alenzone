...
class Home extends CI_Controller
{
    ...
    public function payment_success($method = "", $user_id = "", $amount_paid = "")
    {
        ...
        redirect(base_url('home/my_courses'), 'refresh');
    }
    ...
    {
        $page_data['page_name'] = 'why_us';
        $page_data['page_title'] = get_phrase('why_NTA');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    public function team()
    {
        $page_data['page_name'] = 'team';
        $page_data['page_title'] = get_phrase('Work_Done_By_Our_Finalists');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    public function works()
    {
        $page_data['page_name'] = 'works';
        $page_data['page_title'] = get_phrase('our_work');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    {
        if ($param1 !== "" && $param1 === "send") {
            $data = $this->input->post();
            if (is_array($data)) {
                if (key_exists('submit', $data)) {
                    $name =  $data['name'];
                    $email =  $data['email'];
                    $subject =  $data['subject'];
                    $message =  $data['message'];

                    if ($this->email_model->send_smtp_mail($message, $subject . ' (From ' . $name . ')', 'zikama.sadja@gmail.com', 'saphira@sadjawebtools.com')) {
                        $this->session->set_flashdata('success', get_phrase('Thank_you_for_contacting_us,') . get_phrase('Your_Message_has_been_sent!'));
                        redirect(site_url('home/contact_us'));
                    } else {

                        $this->session->set_flashdata('failure', get_phrase('Opps!_something_is_wrong,') . get_phrase('please_try_again!'));
    ...
    {
        if($param === 'search'){
            $match = $this->input->get('q', TRUE);
            if($match !== '')
            {
                $page_data['page_name'] = 'blog_';
                $page_data['all_articels'] = $this->posts_model->search_post($match);
                $page_data['latest_articels'] = $this->posts_model->get_latest_posts();
                $page_data['page_title'] = $match;
                $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
                return;
        ...
        $page_data['page_name'] = 'blog_';
        $page_data['all_articels'] = $this->posts_model->get_all_posts();
        $page_data['latest_articels'] = $this->posts_model->get_latest_posts();
        $page_data['page_title'] = get_phrase('blog');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    public function post($parm1 = '', $parm2 = '', $parm3 = '', $parm4 = '')
    {
        if($parm1 !== ''&&$parm2 !== ''&&$parm3 !== ''&&$parm4 !== ''){
            $page_data['page_name'] = 'blog_render';
            $page_data['latest_articels'] = $this->posts_model->get_latest_posts();

            $blog_title_slug = implode('_', explode('-', $parm4));
            $page_data['page_title'] = get_phrase($blog_title_slug);
            ...
            $date_year = $parm1;
            $date_month = $montn_array[$parm2];
            $date_day = $parm3;

            $page_data['post_date'] = $date_month . ' ' . $date_day . ', ' . $date_year;


            $blog_title = implode(' ', explode('-', $parm4));
            $blog_link =  $parm1 . '/' . $parm2 . '/' . $parm3 . '/' . $parm4;

            $page_data['post'] = $this->posts_model->get_post('post_link', $blog_link);
